...
def get_aug_config():
    ...
    return trans, scale, rot, do_flip, color_scale,do_skin
...
def augmentation(img, bbox, joint_coord, joint_valid, hand_type, mode, joint_type):
    ...
    if mode == 'train':
        trans, scale, rot, do_flip, color_scale , do_skin= get_aug_config()
    else:
        trans, scale, rot, do_flip, color_scale, do_skin = [0, 0], 1.0, 0.0, False, np.array([1, 1, 1]), False
...
def process_bbox(bbox, original_img_shape, expand_ratio=1):
...
def gen_black_image(image_src: np.ndarray, bbox1, bbox2):
    image_black = np.zeros(image_src.shape, dtype=np.uint8)
    image_black[int(bbox1[1]):int(bbox1[3]), int(bbox1[0]):int(bbox1[2]), :] = \
        image_src[int(bbox1[1]):int(bbox1[3]), int(bbox1[0]):int(bbox1[2]), :].copy()
    image_black[int(bbox2[1]):int(bbox2[3]), int(bbox2[0]):int(bbox2[2]), :] = \
        image_src[int(bbox2[1]):int(bbox2[3]), int(bbox2[0]):int(bbox2[2]), :].copy()
