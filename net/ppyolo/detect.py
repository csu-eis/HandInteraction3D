...
class Detector(object):
    ...
    def postprocess(self, np_boxes, inputs):
        ...
        if self.pred_config.arch in ['Face']:
            ...
            np_boxes[:, 5] *= w
        # 筛选置信度大于阈值且类别为0的包围盒
        filtered_boxes = np_boxes[(np_boxes[:, 1] > self.threshold) & (np_boxes[:, 0] == 0)]

        # 返回[x1, y1, x2, y2]格式的bbox
...
def generate_od_input_from_bgr(image_src, cfg: PredictConfig):
    ...
    target_size = target_size[np.newaxis, :].astype(np.float32)
    scale_factor = scale_factor[np.newaxis, :].astype(np.float32)
...
def generate_od_input2(image_src , cfg: PredictConfig):
...
def generate_od_input(image_path: str, cfg: PredictConfig):
...
def merge_bbox(box1, box2):
    assert box1[0] == box2[0], f"box1 class: {box1[0]} should equal to box2 class: {box2[0]}"
    return np.array([
        box1[0],
        min(box1[1], box2[1]),
        min(box1[2], box2[2]),
...
if __name__ == "__main__":
    model_dir = "../../weights/ppyolo_r18vd_voc"
    cfg = PredictConfig(model_dir)
    image_src, inputs = generate_od_input("../../test.jpg", cfg)
    ...
    bbox = detector.predict(inputs)
    bbox = merge_bbox(bbox[0], bbox[1])
    print(bbox)
